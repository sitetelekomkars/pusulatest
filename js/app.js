...
async function logEvaluationPopup() {
    ...
    const isChatRaw = agentGroup.indexOf('Chat') > -1;
    const isTelesatis = agentGroup.indexOf('Telesatış') > -1;
    
    let displayGroup = agentGroup; // Varsayılan olarak mevcut grup
    
    if (isChatRaw) {
        // Chat için form tipini seç
        ...
        agentGroup = selectedChatType; // Loglama için bu değeri kullan
        displayGroup = selectedChatType;
    } else if (isTelesatis) {
        agentGroup = 'Telesatış'; // Tek bir Telesatış logu kullanıyorsanız
    }
        ...
        <div class="eval-modal-wrapper">
            <div class="score-dashboard">
                <div>
                    ...
                    <div style="font-size:0.8rem; opacity:0.7;">${displayGroup}</div>
                </div>
            ...
            if (isCriteriaBased) {
                ...
                for (let i = 0; i < criteriaList.length; i++) {
                    ...
                    if (isChatRaw) {
                        ...
                        // Genel: Eğer kriter tabanlıysa ama ne chat ne telesatışsa (manuel puanlama ile aynı mantıkta)
                        val = parseInt(document.getElementById(`badge-${i}`).innerText) || 0;
                        note = document.getElementById(`note-${i}`).value;
                    }
                    total += val;
                ...
                // agentGroup'u güncel haliyle gönderiyoruz (Chat-Normal/Chat-Teknik gibi)
                ...
                return { agentName, agentGroup: displayGroup, callId, callDate: formattedCallDate, score: parseInt(score), details: details, feedback, feedbackType: feedbackType }; 
            }
        }
    });
    if (formValues) {
...
async function editEvaluation(targetCallId) {
    ...
    const agentGroup = isChat ? agentGroupRaw : (isTelesatis ? 'Telesatış' : 'Genel'); // Chat/Telesatış/Genel
    ...
    if (isCriteriaBased) {
        ...
        criteriaList.forEach((c, i) => {
            ...
            if (isChat) {
                ...
                // Eğer mevcut puan iyi/orta/kötü ile eşleşmiyorsa, sadece değeri kullan ve aktif yapma
                if (currentVal !== pts && currentVal !== mPts && currentVal !== bPts) {
                    goodActive = mediumActive = badActive = '';
                } else if (currentVal === 0 && bPts > 0) {
                    // Puan 0 ise ve kötü puan 0'dan büyükse, butonu aktif etmeyebiliriz
                    goodActive = mediumActive = badActive = '';
                }
            ...
            if (isCriteriaBased) {
                ...
                for (let i = 0; i < criteriaList.length; i++) {
                    ...
                    if (isChat) {
                        ...
                        // Genel: Kriter tabanlıysa ama ne chat ne telesatışsa
                        val = parseInt(document.getElementById(`badge-${i}`).innerText) || maxPoints;
                        note = document.getElementById(`note-${i}`).value;
                        if (val < maxPoints && !note) {
                            Swal.showValidationMessage(`'${c.text}' için kırılım nedeni zorunludur.`);
                            return false;
