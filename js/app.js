...
function checkAdmin(role) { 
    // Header butonları kaldırıldığı için orijinal ID'ler menüdeki ID'ler ile güncellendi.
    const addCardBtn = document.getElementById('addCardBtn'); 
    const quickEditBtn = document.getElementById('quickEditBtn');
    
    const dropdownAddCard = document.getElementById('dropdownAddCard');
    const dropdownQuickEdit = document.getElementById('dropdownQuickEdit');
    ...
    if(isAdminMode) {
        if(addCardBtn) addCardBtn.style.display = 'flex'; // Orijinal header butonu (varsa)
        if(quickEditBtn) quickEditBtn.style.display = 'flex'; // Orijinal header butonu (varsa)
        
        // Dropdown menüdeki yeni öğeleri göster (HTML'de kaldırıldı, ancak mantık hazır)
        if(dropdownAddCard) dropdownAddCard.style.display = 'flex'; 
        if(dropdownQuickEdit) {
            dropdownQuickEdit.style.display = 'flex';
            dropdownQuickEdit.innerHTML = '<i class="fas fa-pen" style="color:var(--secondary);"></i> Düzenlemeyi Aç';
            dropdownQuickEdit.classList.remove('active');
        }
        ...
        if(dropdownAddCard) dropdownAddCard.style.display = 'none';
        if(dropdownQuickEdit) dropdownQuickEdit.style.display = 'none';
    }
}
function logout() { 
    currentUser = ""; 
...
function renderCards(data) { 
    ...
    data.forEach((item, index) => { 
        ...
        // Orijinal buton ID'leri kullanıldı
...
function toggleEditMode() { 
    ...
    // Orijinal header buton ID'leri (Kullanımdan kaldırılmış olabilir)
    const btnHeader = document.getElementById('quickEditBtn');
    if (btnHeader) {
        if(isEditingActive) { 
            btnHeader.classList.add('active'); 
            btnHeader.innerHTML = '<i class="fas fa-times"></i> Kapat';
        } else { 
            btnHeader.classList.remove('active'); 
            btnHeader.innerHTML = '<i class="fas fa-pen"></i> Düzenle';
        }
    }

    // Menüdeki buton ID'si (Yeni buton)
    const btnDropdown = document.getElementById('dropdownQuickEdit');
    if (btnDropdown) {
        if(isEditingActive) { 
            btnDropdown.classList.add('active'); 
            btnDropdown.innerHTML = '<i class="fas fa-times" style="color:var(--accent);"></i> Düzenlemeyi Kapat';
            ...
            btnDropdown.innerHTML = '<i class="fas fa-pen" style="color:var(--secondary);"></i> Düzenlemeyi Aç';
        } 
    }
    ...
    const { value: formValues } = await Swal.fire({
        ...
        html: `
            <div class="card" style="text-align:left; border-left: 5px solid var(--primary); padding:15px; background:#f8f9fa;">
                ...
                <label style="font-weight:bold;">Okunuşu (Pronunciation)</label><input id="swal-pron" class="swal2-input" style="width:100%; margin-bottom:10px;" value="${s.pronunciation || ''}">
                <label style="font-weight:bold;">İkon Sınıfı (Icon)</label><input id="swal-icon" class="swal2-input" style="width:100%;" value="${s.icon || ''}">
            </div>`,
        width: '700px',
        showCancelButton: true,
...
// KRİTİK DÜZELTME: logEvaluationPopup (Dinamik Form ve Mail Kutusu)
async function logEvaluationPopup() {
    const selectEl = document.getElementById('agent-select-admin');
    const agentName = selectEl.value;
    const selectedOption = selectEl.options[selectEl.selectedIndex];
    // Değerlendirme grubunu geçici olarak al
    let agentGroup = selectedOption.getAttribute('data-group') || 'Genel'; 
    
    // YENİ DÜZELTME: CHAT için dinamik form seçimi
    if (agentGroup === 'Chat') {
        const { value: selectedType } = await Swal.fire({
            title: `${agentName} için Değerlendirme Tipi Seçiniz`,
            input: 'radio',
            inputOptions: {
                'Chat-Normal': 'Müşteri Hizmetleri / İkna',
                'Chat-Teknik': 'Teknik Destek / Hata Çözümü'
            },
            inputValidator: (value) => {
                if (!value) {
                    return 'Bir değerlendirme tipi seçmelisiniz!';
                }
            },
            showCancelButton: true,
        ...
        agentGroup = selectedType; // Seçilen alt grubu ana grup olarak ata
    }

    Swal.fire({ title: 'Değerlendirme Formu Hazırlanıyor...', didOpen: () => Swal.showLoading() });
    
    let criteriaList = [];
    // Dinamik grup ismini kullan (Telesatış, Chat-Normal, Chat-Teknik)
    if(agentGroup.startsWith('Chat') || agentGroup === 'Telesatış') { 
    ...
    contentHtml += `
        <div>
            <div style="margin-bottom: 10px; display: flex; align-items: center; gap: 10px;">
                <input type="checkbox" id="send-email-check" style="width: 16px; height: 16px; margin: 0;">
                <label for="send-email-check" style="font-size: 0.9rem; font-weight: bold; color: var(--accent);">
                    Değerlendirmeyi Temsilciye E-posta Olarak Gönder (Hata Geri Bildirimi)
                </label>
            </div>
    ...
    const { value: formValues } = await Swal.fire({
        ...
        preConfirm: () => {
            ...
            // Yeni Eklendi: Mail Onay Kutusu
            const sendEmail = document.getElementById('send-email-check') ? document.getElementById('send-email-check').checked : false; 
            ...
            if (isCriteriaBased) {
                // Zorunlu Not Kontrolü ve Puan Hesaplama
                ...
                criteriaList.forEach((c, i) => {
                    ...
                    if (val < maxPoints && !noteValue.trim()) {
                        requiredNoteMissing = true;
                    }

                    total += val;
                    totalMax += maxPoints;
                    detailsArr.push({ q: c.text, max: maxPoints, score: val, note: noteValue });
                });
                ...
                const finalScore = totalMax > 0 ? Math.round((total / totalMax) * 100) : 0; // Yüzdelik skor
                
                return { 
                    agentName, 
                    agentGroup, // Chat-Normal veya Chat-Teknik olarak Apps Script'e gidecek
                    callId, 
                    callDate: formattedCallDate, 
                    score: finalScore, // Yüzdelik Skor
                    details: JSON.stringify(detailsArr), 
                    feedback, 
                    shouldSendEmail: sendEmail // YENİ
                };
...
async function editEvaluation(targetCallId) {
    ...
    const currentScorePercentage = evalData.score; // Kayıtlı skor zaten yüzdelik
    ...
    <div class="eval-modal-wrapper" style="border-top: 5px solid #1976d2;">
        <div class="score-dashboard">
            ...
            <div class="score-circle-outer" id="score-ring">
                <div class="score-circle-inner" id="live-score">${currentScorePercentage}</div>
            </div>
        </div>
        <div class="eval-header-card">
            <div>
    ...
    if (isCriteriaBased) {
        ...
        criteriaList.forEach((c, i) => {
            ...
            if (!oldItem && oldDetails[i] && oldDetails[i].score !== undefined) {
            ...
            let currentVal = parseInt(oldItem.score) || 0;
            ...
            <div class="criteria-row" id="row-${i}">
                ...
                <input type="text" id="note-${i}" class="note-input" placeholder="Kırılım nedeni..." style="display:${currentVal < pts ? 'block' : 'none'};" value="${oldItem.note || ''}">
            </div>`;
        });
        contentHtml += `</div>`;
    } else {
        // Manuel Puanlama formu (kriter bazlı değilse)
        contentHtml += `
            <div style="padding:15px; border:1px dashed #ccc; background:#fff; border-radius:8px; text-align:center;">
                <label style="font-weight:bold;">Manuel Puan</label><br>
        ...
        didOpen: () => {
            ...
            if(isCriteriaBased) {
                ...
                criteriaList.forEach((c, i) => {
                    ...
                    if (noteInp && parseInt(document.getElementById(`slider-${i}`).value) < pts) {
                        noteInp.required = true; 
                    } else if (noteInp) {
                         noteInp.required = false;
                    }
                });
            } else {
                let initialPercentage = parseInt(document.getElementById('eval-manual-score').value) || 0;
                const ringEl = document.getElementById('score-ring');
                const liveScoreEl = document.getElementById('live-score');
                liveScoreEl.innerText = initialPercentage;
                let color = initialPercentage >= 95 ? 'var(--success)' : (initialPercentage < 50 ? 'var(--accent)' : (initialPercentage < 85 ? 'var(--warning)' : 'var(--secondary)'));
                ringEl.style.background = `conic-gradient(${color} ${initialPercentage}%, #444 ${initialPercentage}%)`;
            }
            document.getElementById('eval-feedback').value = evalData.feedback || '';
            if(!isCriteriaBased && document.getElementById('eval-details')) {
                document.getElementById('eval-details').value = (typeof evalData.details === 'string' ? evalData.details : '');
            }
        },
        preConfirm: () => {
            const callId = document.getElementById('eval-callid').value;
