...
async function checkSession() {
    ...
    if (session && session.user) {
        ...
        currentUser = profile.username || user.user_metadata?.username || user.email;
        isAdminMode = (profile.role === 'admin' || profile.role === 'locadmin');
        isLocAdmin = (profile.role === 'locadmin');
        ...
        // LocalStorage fallback (bazı eski kodlar için gerekebilir)
        localStorage.setItem("sSportUser", currentUser);
        localStorage.setItem("sSportRole", activeRole);
        localStorage.setItem("sSportGroup", profile.group || "");
...
async function girisYap() {
    const uEmail = document.getElementById("usernameInput").value.trim();
...
async function sendHeartbeat() {
    ...
    try {
        ...
        if (data && data.status === 'Kicked') {
            await sb.from('profiles').update({ status: 'Aktif' }).eq('id', user.id);
            Swal.fire({
