...
function populateMonthFilterFull() {
    const selectIds = ['q-dash-month','q-admin-month']; // Sadece yeni filtre
    const now = new Date();
...
async function fetchEvaluationsForAgent(forcedName, silent=false) {
    ...
    const monthSelect = document.getElementById('q-admin-month');
    ...
    try {
        const response = await fetch(SCRIPT_URL, {
            method: 'POST', headers: { "Content-Type": "text/plain;charset=utf-8" },
            body: JSON.stringify({ action: "fetchEvaluations", targetAgent: targetAgent, targetGroup: targetGroup, month: targetMonth, username: currentUser, token: getToken() })
        });
        const data = await response.json();
...
async function editEvaluation(targetCallId) {
    ...
    // Çağrı tarihini date input için ISO formata çevir
    let callDateIso = '';
    if (evalData.callDate) {
        const parts = String(evalData.callDate).split('.');
        if (parts.length === 3) {
            const [dd, mm, yyyy] = parts;
            callDateIso = `${yyyy}-${mm.padStart(2,'0')}-${dd.padStart(2,'0')}`;
        }
    }
    ...
    const { value: formValues } = await Swal.fire({
        ...
        allowEscapeKey: false,
        didOpen: () => {
        // Mevcut skor hesaplamaları
        if (isTelesatis) window.recalcTotalSliderScore();
        else if (isChat) window.recalcTotalScore();

        // Çağrı tarihini düzenlemek için input ekle
        try {
            const headerCard = document.querySelector('.eval-header-card');
            if (headerCard && !document.getElementById('eval-calldate')) {
                const wrap = document.createElement('div');
                wrap.innerHTML = `
                    <label>Çağrı Tarihi</label>
                    <input id="eval-calldate" type="date" class="swal2-input" style="height:35px; width:100%;" />
                `;
                headerCard.appendChild(wrap);
                if (callDateIso) {
                    const dateInput = document.getElementById('eval-calldate');
                    if (dateInput) dateInput.value = callDateIso;
                }
            }
        } catch (e) {
            console.error('Call date alanı eklenirken hata:', e);
        }
    },
        preConfirm: () => {
            const callId = (document.getElementById('eval-callid').value || '').trim();
            ...
            if (!callId) {
                Swal.showValidationMessage('Call ID boş bırakılamaz.');
                return false;
            }

            // Tarih alanını dd.MM.yyyy formatına çevir
            const callDateRaw = document.getElementById('eval-calldate') ? document.getElementById('eval-calldate').value : '';
            let formattedCallDate = evalData.callDate || '';
            if (callDateRaw) {
                const parts = callDateRaw.split('-'); // YYYY-MM-DD
                if (parts.length === 3) {
                    const [yyyy, mm, dd] = parts;
                    formattedCallDate = `${dd}.${mm}.${yyyy}`;
                }
            }
            ...
            if (isCriteriaBased) {
                ...
                const manualScoreInput = document.getElementById('eval-manual-score-edit');
                const manualDetailsInput = document.getElementById('eval-details-edit');
                const manualScore = Number(manualScoreInput ? manualScoreInput.value : 0) || 0;
                const manualDetails = manualDetailsInput ? manualDetailsInput.value : '';
                ...
                return {
                    ...
                    callDate: formattedCallDate,
                    score: manualScore,
                    details: JSON.stringify([{ criteria: 'Serbest', score: manualScore, note: manualDetails }]),
                    feedback
                };
            }
        }
